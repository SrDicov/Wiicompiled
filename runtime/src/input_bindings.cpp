...
void Apply(PADStatus* statuses) noexcept {
    ...
    const bool blocked = PADIsInputBlocked();
    for (uint32_t port = 0; port < PAD_CHANMAX; ++port) {
        ...
        for (size_t control = 0; control < kControls.size(); ++control) {
            ...
            if (info.analog != 0) {
                const auto scaled = static_cast<uint8_t>(std::clamp(value, 0.0, 1.0) * 255.0);
                uint8_t& target =
...
std::string DefaultDolphinConfigPath() noexcept {
    ...
    const std::filesystem::path local = std::filesystem::path("GCPadNew.ini");
    if (std::filesystem::exists(local, ec)) {
        return std::filesystem::absolute(local, ec).string();
    }
    return local.string();
}
...
int ImportDolphinConfig(const std::string& path, int padIndex, uint32_t port, std::string& summary,
    ...
    std::string device;
    if (!InputExpr::ReadDolphinConfig(path, padIndex, controls, device, error)) {
        return -1;
    }
