...
double Eval(const Node& node, const InputSource& source) {
    ...
    case Kind::Div: {
        ...
        return rhs == 0.0 ? 0.0 : Arg(node, 0, source) / rhs;
    }
    ...
    case Kind::FnClamp:
        return std::clamp(Arg(node, 0, source), Arg(node, 1, source), Arg(node, 2, source));
    ...
    case Kind::FnDeadzone: {
        ...
        const double dz = Arg(node, 1, source);
    ...
    case Kind::FnTap: {
        ...
        const unsigned desired =
            node.args.size() == 3 ? static_cast<unsigned>(Arg(node, 2, source) + 0.5) : 2u;
        if (input < kConditionThreshold) {
            node.released = true;
...
double Expression::Evaluate(const InputSource& source) const {
    return m_root ? Eval(*m_root, source) : 0.0;
}
...
bool ReadDolphinConfig(const std::string& path, int padIndex,
    ...
    if (!found) {
        error = wanted + " not found in " + path;
        return false;
    }
    return true;
}
